...
export function newsMetadata(locale: Locale, country?: string): Metadata {
  ...
  const title = `${label} — ${message(locale, "news")} | HomoLudens`;
  ...
  return {
    ...
    openGraph: {
      title, description, type: "website", url: canonical, locale: OPEN_GRAPH_LOCALES[locale], siteName: "HomoLudens",
      images: [{ url: SOCIAL_PREVIEW_PATH, width: 1200, height: 630, alt: "HomoLudens", type: "image/png" }],
    },
    twitter: { card: "summary_large_image", title, description, images: [{ url: SOCIAL_PREVIEW_PATH, alt: "HomoLudens" }] },
...
export function articleMetadata(article: NewsArticle, locale: Locale): Metadata {
  ...
  return {
    title: `${text.title} | HomoLudens`, description: text.description,
    ...
    openGraph: {
      ...
      siteName: "HomoLudens", images: [{ url: image, width: 1200, height: 630, alt: text.title, type: "image/png" }],
    },
...
export function articleStructuredData(article: NewsArticle, locale: Locale) {
  ...
  return {
    ...
    publisher: { "@type": "Organization", name: "HomoLudens", url: ODDSFRONT_URL, logo: { "@type": "ImageObject", url: `${ODDSFRONT_URL}/brand/oddsfront-app-512-v1.png`, width: 512, height: 512 } },
...
export function topicStructuredData(topic: NewsCategory, locale: Locale, articles: NewsArticle[]) {
  ...
  return { "@context":"https://schema.org", "@type":"CollectionPage", name:categoryLabel(topic,locale), url, inLanguage:locale,
    isPartOf:{ "@type":"WebSite", name:"HomoLudens", url:ODDSFRONT_URL },
