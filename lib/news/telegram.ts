...
export const TELEGRAM_INTERVAL_MS = SOCIAL_PUBLICATION_INTERVAL_MS;
...
export function freshEditionArticles(articles: NewsArticle[], sent: readonly string[], now = Date.now()): NewsArticle[] {
  ...
  let pool = fresh.filter(article => !sent.includes(article.id)).slice(0, NEWS_EDITION_SIZE * 3);
  ...
  // Each social slot can select another story from the same two-hour edition.
  if (previous) {
  ...
  return pool.toSorted((a, b) => newsPublicationPriority(b) - newsPublicationPriority(a) || Date.parse(b.publishedAt) - Date.parse(a.publishedAt)).slice(0, NEWS_EDITION_SIZE);
}
