...
function StoryArt({ article, featured = false }: { article: NewsArticle; featured?: boolean }) {
  ...
  return <div className={`${styles.art} ${featured ? styles.featuredArt : ""}`} data-country={primary} data-cover-state={coverState} aria-hidden="true">
    ...
    {coverState === "loaded" ? <span className={styles.coverCredit}>Image · {mediaSource?.publisher}</span> : <><span className={styles.artTop}>ODDSFRONT / {translate("World desk")}</span><span className={styles.artLocation}>{primary.length === 2 ? <CountryFlag code={primary}/> : null}{primary.length === 2 ? country(primary) : t("news")}</span><span className={styles.artCode}>{primary}</span><span className={styles.artBottom}>{article.topics.slice(0,2).map(translate).join(" · ")}</span></>}
  </div>;
}
...
function LiveMarketCard({ event, fresh }: { event: ConflictPreviewEvent; fresh: boolean }) {
  ...
  return <article className={styles.marketCard} data-testid="news-related-market"><div className={styles.marketKicker}><span className={fresh?styles.liveDot:undefined}/>{t(fresh?"live":"updated")}<span>{event.countryCodes.slice(0,3).map(code=><CountryFlag key={code} code={code}/>)}</span></div><div className={styles.marketHeadline}>{event.imageUrl?<span className={styles.marketImage} aria-hidden="true"><Image src={`/api/market-image/${encodeURIComponent(event.id)}`} alt="" width={42} height={42} loading="lazy" onError={image=>{image.currentTarget.parentElement?.setAttribute("hidden","");}} data-market-image/></span>:null}<h3>{translate(event.title)}</h3></div><div className={styles.marketOdds} dir="ltr"><div><span>{t("yes")}</span><strong>{event.yesOdds}%</strong></div><div><span>{t("no")}</span><strong>{event.noOdds}%</strong></div></div><div className={styles.marketVolume}>{t("volume")} <b dir="ltr">{new Intl.NumberFormat(locale,{style:"currency",currency:"USD",notation:"compact",maximumFractionDigits:1}).format(event.marketVolume??event.volume)}</b></div><div className={styles.marketActions}>{track?<a href={track} target="_blank" rel="noreferrer">{t("track")}</a>:null}{market?<a href={market} target="_blank" rel="noreferrer">{t("market")}<ArrowUpRight size={14}/></a>:null}</div></article>;
}
...
export function NewsArticleView({ article, initialFeed }: { article: NewsArticle; initialFeed: ConflictPreviewFeed }) {
  ...
  return <NewsChrome><Link className={styles.back} href={`/news?lang=${locale}`} prefetch={false}><ArrowLeft size={15}/>{t("back")}</Link><div className={styles.articleGrid}><article className={styles.article} lang={translated?locale:"en"} dir={localeDirection(translated?locale:"en")}>
    ...
    <StoryArt key={article.slug} article={article} featured/>
