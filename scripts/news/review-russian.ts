...
if (!response.ok) throw new Error("Market questions unavailable for Russian editorial review");
for (const candidate of telegramCandidates(articles, await response.json(), [])) texts.add(candidate.event.title);
...
const pending = [...texts].filter(text => !cache[russianEditorialKey(text)]);
if (pending.length) {
